...
public class JunkCode {
	...
	public int addPersonToDB(Person p) throws ClassNotFoundException, SQLException {
		...
		String queryStatement = String.format("SELECT MAX(personID) FROM Person");
		String updateStatement = String.format("INSERT INTO Person VALUES ('',%s,'%s',%s,'%s','%s')",navn,phoneNumber,brukerNavn, password);		
		db.makeSingleUpdate(updateStatement);
		ResultSet rs = db.makeSingleQuery(queryStatement);
